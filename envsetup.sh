...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^zap_") ; then
       ...
       export BUILD_NUMBER=$((date +%s%N ; echo $ZAP_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       ZAP_BUILD=
    fi
    export ZAP_BUILD
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var ZAP_VERSION)
        ZIPFILE=zap-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.zap.device=$ZAP_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $ZAP_BUILD, run away!"
    fi
}
...
function cmremote()
{
    ...
    ZAPUSER=$(git config --get review.review.cyanogenmod.org.username)
    if [ -z "$ZAPUSER" ]
    then
        git remote add cmremote ssh://review.cyanogenmod.org:29418/$GERRIT_REMOTE
    else
        git remote add cmremote ssh://$ZAPUSER@review.cyanogenmod.org:29418/$GERRIT_REMOTE
    fi
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/zap/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
